...
center maps to safe (left) / work (center) / restricted (right).

Press 'w' in main.py to cycle layouts: split → all safe → all work → all restricted.
"""
...
def person_frame_norm(person: TrackedPerson, width: int, height: int) -> Tuple[float, float]:
    """Bbox center in normalized frame coords."""
...
class ZoneMonitor:
    """Assigns persons to frame zones; supports runtime layout cycling."""
    ...
    def __init__(
        ...
        if self._base_zones:
            print(f"[ZoneMap] Layout: {LAYOUT_LABELS[self._layout_mode]}")
    ...
    def cycle_layout(self) -> str:
        """Cycle split → all safe → all work → all restricted → split."""
...
def draw_zones_overlay(frame: np.ndarray, frame_data: FrameData) -> None:
    """Draw zone bands and current layout label."""
